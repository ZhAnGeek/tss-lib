...
// 0 if f == rhs
// 1 if f > rhs
func (fp *Field) Cmp(rhs *Field) int {
	gt := 0
	lt := 0
	for i := 3; i >= 0; i-- {
		gt |= int(compareUint64(fp.Value[i], rhs.Value[i])) &^ lt
		lt |= int(compareUint64(rhs.Value[i], fp.Value[i])) &^ gt
	}
	return gt - lt
}
...
func (f *Field) SetBytes(input *[FieldBytes]byte) (*Field, error) {
	...
	d0f := new(Field).SetLimbs(&d0)
	mof := new(Field).SetLimbs(&f.Params.Modulus)
	if d0f.Cmp(mof) != -1 {
...
func (f *Field) SetBigInt(bi *big.Int) *Field {
	...
	t.FillBytes(buffer[:])
	copy(buffer[:], pasta.ReverseScalarBytes(buffer[:]))
...
func (f *Field) BigInt() *big.Int {
	buffer := f.Bytes()
	return new(big.Int).SetBytes(pasta.ReverseScalarBytes(buffer[:]))
}
