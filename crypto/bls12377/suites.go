...
func GetBLSSignatureSuiteG1() []byte {
	...
	H2cSuiteID := getHashTOCurveSuiteID([]byte("BLS12377G1"), HashID, []byte("SSWU"), []byte("RO"))
	ScTag := []byte("POP")
...
func GetBLSSignatureSuiteG2() []byte {
	HashID := getHashID([]byte("XMD"), []byte("SHA-256"))
	H2cSuiteID := getHashTOCurveSuiteID([]byte("BLS12377G2"), HashID, []byte("SSWU"), []byte("RO"))
	ScTag := []byte("POP")
	return getBLSSignatureCipherSuiteID(H2cSuiteID, ScTag)
}
...
func I2OSP(x, xLen int) ([]byte, error) {
	...
	if xB.Cmp(common.Zero) == -1 || xB.Cmp(new(big.Int).Lsh(big.NewInt(1), uint(8*xLen))) != -1 {
		return nil, fmt.Errorf("I2OSP overflow %s, %d", xB.String(), xLen)
	}
...
func expandMessageXmd(msg, DST []byte, lenInBytes int) ([]byte, error) {
	...
	if ell > 255 {
		return nil, fmt.Errorf("expand_message: ell=%d out of range", ell)
	}
...
func HashToPointG1(message []byte) (*bls.G1Affine, error) {
	DST := GetBLSSignatureSuiteG1()
	Fs, err := hashToField(message, 1, DST, G1Curve().P, 1, 64)
	if err != nil {
		return nil, err
	}
	FBytes := make([]byte, 48)
	Fs[0][0].FillBytes(FBytes)
	dG1, err := bls.HashToG1(FBytes, DST)
	if err != nil {
		return nil, err
	}
	return &dG1, nil
...
func HashToPointG2(message []byte) (*bls.G2Affine, error) {
	DST := GetBLSSignatureSuiteG2()
	Fs, err := hashToField(message, 1, DST, G2Curve().P, 1, 64)
	if err != nil {
		return nil, err
	}
	FBytes := make([]byte, 96)
	Fs[0][0].FillBytes(FBytes)
	dG2, err := bls.HashToG2(FBytes, DST)
	if err != nil {
		return nil, err
	}
	return &dG2, nil
