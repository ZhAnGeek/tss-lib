...
func TestE2EConcurrent(t *testing.T) {
	...
	for j, pID := range oldPIDs {
		params := tss.NewReSharingParameters(tss.S256(), oldP2PCtx, newP2PCtx, pID, testParticipants, threshold, newPCount, newThreshold, 0, nil)
...
presigning:
	...
	for j, signPID := range presignPIDs {
		params := tss.NewParameters(tss.S256(), presignP2pCtx, signPID, len(presignPIDs), newThreshold, false, 0, nil)
...
signing:
	...
	for j, signPID := range signPIDs {
		params := tss.NewParameters(tss.S256(), signP2pCtx, signPID, len(signPIDs), newThreshold, false, 0, nil)
