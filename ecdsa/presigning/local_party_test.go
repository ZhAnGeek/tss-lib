...
func TestR2RConcurrent(t *testing.T) {
	...
	// @Presign 1
	for i := 0; i < N; i++ {
		params := tss.NewParameters(tss.EC(), p2pCtx, signPIDs[i], N, threshold, false, 0)
...
signing:
	...
	il, _, errorDerivation := DerivingPubkeyFromPath(ctx, keys[0].ECDSAPub, chainCode, []uint32{12, 209, 3}, tss.EC())
