...
func TestR2RConcurrent(t *testing.T) {
	...
	// @Presign 1
	for i := 0; i < N; i++ {
		params := tss.NewParameters(tss.S256(), p2pCtx, signPIDs[i], N, threshold, false, 0, nil)
...
func TestR2RWithIdentification(t *testing.T) {
	...
	// @Presign 1
	for i := 0; i < N; i++ {
		params := tss.NewParameters(tss.S256(), p2pCtx, signPIDs[i], len(signPIDs), threshold, true, 0, nil)
