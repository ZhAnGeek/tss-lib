...
func NewLocalParty(
	...
	p := &LocalParty{
		...
		LocalParty: frost.NewLocalParty(params, out, frostEnd).(*frost.LocalParty),
		params:     params,
		temp:       localTempData{},
		data:       data,
		out:        out,
...
func (p *LocalParty) Start(ctx context.Context) *tss.Error {
	go func() {
		...
		case save := <-p.frostEnd:
			p.data.LocalSecrets = keygen.LocalSecrets{
				Xi:      save.LocalSecrets.Xi,
				ShareID: save.LocalSecrets.ShareID,
			}
			p.data.Ks = save.Ks
			p.data.BigXj = save.BigXj
			p.data.ECDSAPub = save.PubKey
			p.end <- p.data
		}
	}()
	err := p.LocalParty.Start(ctx)
	if err != nil {
		return err
	}
...
func (p *LocalParty) Update(ctx context.Context, msg tss.ParsedMessage) (ok bool, err *tss.Error) {
	ok1, err := p.LocalParty.Update(ctx, msg)
	if err != nil {
		return ok1, err
	}
	ok2, err := tss.BaseUpdatePool(ctx, p, msg, TaskName)
	return ok1 || ok2, err
}
