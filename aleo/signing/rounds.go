...
func (round *base) getSSID(ctx context.Context) ([]byte, error) {
	...
	BigXjList, err := crypto.FlattenECPoints(round.key.PkSigShares)
	if err != nil {
		return nil, round.WrapError(errors.New("read BigXj failed"), round.PartyID())
	}
	ssidList = append(ssidList, BigXjList...) // PkSigShares
	BigXjList, err = crypto.FlattenECPoints(round.key.PrSigShares)
	if err != nil {
		return nil, round.WrapError(errors.New("read BigXj failed"), round.PartyID())
	}
	ssidList = append(ssidList, BigXjList...)                                                // PrSigShares
	ssidList = append(ssidList, []*big.Int{round.key.Address.X(), round.key.Address.Y()}...) // Address
	ssidList = append(ssidList, big.NewInt(int64(round.number)))                             // round number
	ssidList = append(ssidList, round.temp.PkSigDelta)
	ssidList = append(ssidList, round.temp.PrSigDelta)
