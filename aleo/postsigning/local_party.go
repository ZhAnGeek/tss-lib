...
type (
	...
	localMessageStore struct {
		psignRound1Messages []tss.ParsedMessage
	}
...
func NewLocalParty(
	...
	// msgs init
	p.temp.psignRound1Messages = make([]tss.ParsedMessage, partyCount)

	// temp data init
	p.temp.ssidNonce = nonce
	p.temp.challenge = challenge
	p.temp.ri = signData.UnmarshalRi()
	p.temp.ssid = signData.UnmarshalSsid()

	p.temp.PkSigDerivationDelta = PkSigDerivationDelta
	return p
}
...
func (p *LocalParty) StoreMessage(ctx context.Context, msg tss.ParsedMessage) (bool, *tss.Error) {
	...
	case *PSignRound1Message:
		p.temp.psignRound1Messages[fromPIdx] = msg
